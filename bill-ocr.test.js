...
test("chỉ cộng các giá trị âm thuộc dòng mã giảm", () => {
  const parsed = parseBillText(`
    Đơn của Mai
    Bánh mì 121.000đ
    Tổng tạm tính 121.000đ
    Ưu đãi dự kiến 26.000đ
    Giảm phí ship -10.000đ
    GrabVIP Benefit −4.840đ
    Voucher freeship –9.000đ
  `);
...
test("không biến chênh lệch Apple Pay thành mã giảm giá", () => {
  const parsed = parseBillText(`
  ...
  assert.equal(parsed.subtotal, 159000);
  assert.equal(parsed.discount, 0);
  assert.equal(parsed.totalPayable, 117000);
});

test("không suy ngược mã giảm từ tổng thanh toán OCR sai", () => {
  const parsed = parseBillText(`
    Tổng tạm tính 121.000đ
    Phí áp dụng 26.000đ
    Tổng tiền phải trả 22.160đ
  ...
  assert.equal(parsed.discount, 0);
  assert.equal(parsed.totalPayable, 22160);
});
...
test("chỉ giữ người thực sự có món và bỏ chữ giao diện bị nhận nhầm thành tên", () => {
  const parsed = parseBillText(`
    Võ Diễm
    1 phần
    1x Americano Nước Dừa
    ...
    Xóa
    1 phần
    Tổng tạm tính 153.000đ
  `);

  assert.deepEqual(parsed.people, ["Võ Diễm", "Nguyễn Minh Nhật", "Lê Tiến"]);
});
